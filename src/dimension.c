...
void
dimension_update(Hypertable *ht, Name dimname, DimensionType dimtype, Datum *interval,
...
ts_dimension_set_num_slices(PG_FUNCTION_ARGS)
{
	...
	dimension_update(ht, colname, DIMENSION_TYPE_CLOSED, NULL, NULL, &num_slices, NULL);
...
ts_dimension_set_interval(PG_FUNCTION_ARGS)
{
	...
	dimension_update(ht, colname, DIMENSION_TYPE_OPEN, &interval, &intervaltype, NULL, NULL);
	ts_hypertable_func_call_on_data_nodes(ht, fcinfo);
	ts_cache_release(hcache);
