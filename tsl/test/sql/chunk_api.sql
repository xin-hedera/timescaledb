...
-- Check underlying pg_statistics table (looking at all columns except starelid, which changes depending on how many tests are run before this)
RESET ROLE;
...
EXPLAIN VERBOSE SELECT device, AVG(temp) FROM costtable WHERE temp > 90 AND device < 10 GROUP BY device;
