...
{
	Oid arg_namespace = GetSysCacheOid1(NAMESPACENAME, input_strings[ENCODED_TYPE_NAMESPACE]);
	Oid result;
...
{
	Oid proc_namespace = GetSysCacheOid1(NAMESPACENAME, input_strings[ENCODED_OP_NAMESPACE]);
								  ...
								  const char *node_name)
{
	...
	chunk = ts_chunk_get_by_id(cdn->fd.chunk_id, 0, true);
...
chunk_api_fetch_next_colstats_tuple(FuncCallContext *funcctx)
{
	...
	while (tuple == NULL && ctx->chunk_oids != NIL)
	{
		...
		Chunk *chunk = ts_chunk_get_by_relid(relid, 0, true);
...
/*
 * Fetch and locally update a distributed hypertable stats
 * by table id.
 *
 * This function reconstructs fcinfo argument of the
 ...
 * _timescaledb_internal.get_chunk_colstats() remotely
 * (depending on the passed `col_stats` bool).
 */
static void
